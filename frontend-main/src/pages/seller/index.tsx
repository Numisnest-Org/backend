...
const SellerPage = () => {
    ...
    const fetchSellerPageData = async () => {
      try {
        const [
          ...
          sellerItemsResponse,
          sellerFeaturedResponse,
        ] = await Promise.all([
          axiosPublic.get(`duo/collector/get-seller/${id}`),
          axiosPublic.get(`duo/collector/seller-items/${id}?&limit=24`),
  ...
  return (
    <VisitorLayout>
      ...
      <Collections sellerCollectionData={sellerCollection} sellerId={data?._id} sellerFirstname={data?.first_name} />
